...
def submit():
 if request.method== 'POST':
  ...
  if(btn.lower()=="update"):
      cur = mysql.connection.cursor()
      cur.execute("update parent set fname='"+t2+"',lname='"+t3+"',dob='"+t4+"',phone="+t5+",mobile="+t6+",status='"+t7+"' where parent_id="+t1+"")
      mysql.connection.commit()
      cur.close()
      return redirect(url_for('index'))
  if(btn.lower()=="allsearch"): 
      ...
      data=cur.fetchall()
      cur.close()
      return render_template('Psearch.html',data=data)
  ...
  if(btn.lower()=="specialsearch"):
      ...
      cursor.execute("select * from parent where "+col+"="'"+tspsrc+"')
