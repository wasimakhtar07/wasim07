...
def submit():
 if request.method== 'POST':
  ...
  if(btn=="Delete"):
   cur = mysql.connection.cursor()
   cur.execute("delete from grade where grade_id='"+t1+"'")
   mysql.connection.commit()
   cur.close()
   return redirect(url_for('index'))
  #end delete 
  if(btn.lower()=="allsearch"): 
      ...
      data=cur.fetchall()
      cur.close()
      return render_template('Gsearch.html',data=data)
  if(btn.lower()=="update"):
      cur = mysql.connection.cursor()
      cur.execute("update  grade set name='"+t2+"',desc1='"+t3+"'where grade_id="+t1+"")
      mysql.connection.commit()
      cur.close()
      return redirect(url_for('index'))
  ...
  if(btn.lower()=="specialsearch"):
      ...
      cursor.execute("select * from grade where "+col+"="'"+tspsrc+"')
